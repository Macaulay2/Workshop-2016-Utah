...
inverseOfMapSimis: MinorsLimit => 0, so we now compute syzygies instead.
                   If this doesn't terminate quickly, you may want to try increasing the option MinorsLimit.
...
i5 : (time g = inverseOfMap(phi, Verbose=>false));
     ...
     -- used 0.375 seconds
i6 : (time f = inverseOfMap(phi, Verbose=>false, MinorsLimit=>0));
