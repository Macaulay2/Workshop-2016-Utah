...
\section{Comments on speed of functions}
\noindent
...
Indeed, in this example, {\tt Cremona} is substantially faster.
{\scriptsize
\color{blue}\begin{verbatim}
...
{\color{black}\normalsize}
Note in the above example, increasing the option {\tt MinorSize} makes {\tt inverseOfMap} much slower. 
...
{\color{black}\normalsize
On the other hand, when the source and target are  not projective space, or the map does not have maximal linear rank, in our experience, our functions appear to be faster (although sometimes changing the options {\tt Strategy}, {\tt HybridLimit}, {\tt MinorSize} and {\tt QuickRank}, can make a large difference).  
We include an example where the map does not have maximal linear rank.  
}
{\scriptsize
\color{blue}\begin{verbatim}
i1 : loadPackage "Cremona"; loadPackage "RationalMaps"; 
...
i5 : (time g = inverseOfMap(phi, AssumeDominant=>true,CheckBirational=>false, Verbose=>false));
     ...
     -- used 0.640625 seconds
i6 : (time h = inverseMap(phi)); -- Function from "Cremona"
...
{\color{black}\normalsize
In this final example, for instance, setting {\tt MinorsCount=>0} makes {\tt inverseOfMap} very slow.
...
\begin{itemize}
  ...
  \item{} Ana-Maria Castravet and Zhuang He communicated to us that they used our package to compute the inverse of a rational map from $\bP^3$ to $\bP^3$.  This map was given by 4 degree 13 forms, with 482, 466, 466, and 466 terms respectively.  Computing the inverse of this map took several hours to compute, but it was doable.  As is frequently the case in Macaulay2, working over a finite field can make computations substantially faster than working over $\bQ$.  \cite{} 
\end{itemize}
