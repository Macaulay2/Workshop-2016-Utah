...
We now include an example where the map does not have maximal linear rank.  %In this example, when we set {\tt MinorsCount=>0}, the {\tt RationalMaps} function is also much much slower. 
}
{\scriptsize
\color{blue}\begin{verbatim}
i1 : loadPackage "Cremona"; loadPackage "RationalMaps"; 
...
We conclude with discussions of the limits of this pacakge.
 ...
 Work of O.~Gabber shows that if $f : \bP^n \to \bP^n$ is defined by forms of degree $d$, then its inverse can be defined by forms of degree $d^{n-1}$, \cite{}.  This bound is sharp, as the map
    \[
        (x_0^d : x_1 x_0^{d-1} : x_2 x_0^{d-1} - x_1^d : \dots : x_n x_0^{d-1} - x_{n-1}^d)
    \]
    has inverse given by forms of degree $d^{n-1}$, see \cite{}.  Thus we might expect that this family of maps would be good to explore to see the limits of {\tt RationalMaps}.  We ran these examples with the following code.
}
{\scriptsize
\color{blue}
\begin{verbatim}
...
{\color{black}\normalsize    
When $n = 3$ (we are working on $\bP^3$) here is a table showing the computation time for the inverse for various $d$ at least one one system.  The degrees are those we would expect in this example (when $d = 100$, the degree of the forms in the inverse is $10000$).  Note {\tt Cremona} has nearly identical performance for these examples in $\bP^3$ ($n = 3$), but has slower as we increase the dimension.  
\begin{center}
...
\begin{center}
  ...
  \end{tabular}
\end{center}
Note the last case took more than 4 hours.


Finally, we mention that Ana-Maria Castravet and Zhuang He communicated to us that they used {\tt RationalMaps} to compute the inverse of a rational map from $\bP^3$ to $\bP^3$.  This map was given by 4 degree 13 forms, with 485, 467, 467, and 467 terms respectively.  Computing the inverse of this map took several hours to compute, but it was successful.  \cite{} % As is frequently the case in Macaulay2, working over a finite field can make computations substantially faster than working over $\bQ$ (in this case, it seemed to approximately halve the computation time).  \cite{} 
